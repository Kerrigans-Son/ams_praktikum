    ...
    def __init__(self,scope,trainer):
        with tf.variable_scope(scope):
            ...
            if scope != 'global':
				...
				# Policy Loss Funktion: L = -log(pi(s)) * A(s) - ß* H(pi)
    ...
    def work(self,max_episode_length,gamma,sess,coord,saver):
        ...
        with sess.as_default(), sess.graph.as_default():                 
            while not coord.should_stop():
                ...
                episode_values = []
                episode_frames = []
                episode_reward = 0		#Belohnung
                episode_step_count = 0	#Anzahl der Episodenschritte
                d = False
                ...
                while not episode_end:
                    ...
                    base_action = sample_dist(base_action_dist)		#waehle Aktion aus, die der Agent machen will
                    arg_screen_x = sample_dist(screen_x_dist)		# X Koordinate
                    arg_screen_y = sample_dist(screen_y_dist)		# Y Koordinate
                    arg_screen2_x = sample_dist(screen2_x_dist)		# X Koordinate fuer Select
                    arg_screen2_y = sample_dist(screen2_y_dist)		# Y Koordinate fuer Select
                    arg_select_point_act = sample_dist(select_point_act_dist)	# fuer select_rect
                    arg_select_add = sample_dist(select_add_dist)				# fuer select Funktionen
                    arg_control_group_act = sample_dist(control_group_act_dist)	# fuer select Funktionen mit Gruppen
                    arg_control_group_id = sample_dist(control_group_id_dist)	# fuer select Funktionen mit Gruppen
                    arg_select_unit_id = sample_dist(select_unit_id_dist)		# fuer select Funktionen mit Einheit
                    arg_select_unit_act = sample_dist(select_unit_act_dist)		# fuer select Funktionen mit Einheit
                    arg_queued = sample_dist(queued_dist)

                    # 17 mögliche Aktionen die eine Agent machen kann (Actionsnummer ID, arguments: Übergabe der Argumenten)
                    if base_action == 0:
                        # 0/no_op
                        action_id = 0
                        arguments = []
                    ...
                    a = actions.FunctionCall(action_id, arguments)		# Aufruf der Aktion
                    obs = self.env.step(actions=[a])					# Ausfuehrung in der Umgebung
                    ...
                    episode_reward += r		# Belohnung
                    s_screen = s1_screen
                    s_nonspatial = s1_nonspatial                 
                    total_steps += 1		# Schritt hinzufuegen
                    episode_step_count += 1 # Schritt hinzufuegen
				...
				# Ausgabe in der Konsole der Werte:
				...
				# aktualisiere den lokale Netzwerk  
...
def main():
    ...
    with tf.device("/cpu:0"): 
        ...
        master_network = AC_Network('global',None) # erstellt globale Netzwerk 
       # num_workers = multiprocessing.cpu_count() # Set workers to number of available CPU threads
        num_workers = 1		#erstelle nur eine Agent 
        workers = []
